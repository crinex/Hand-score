//
// Originally derived from AnemllCore (https://github.com/Anemll/Anemll),
// MIT License. Vendored and renamed to HandScoreCore for the HAND-Score
// iOS benchmark release. See LICENSE-AnemllCore at the package root for
...
@preconcurrency public final class InferenceManager: @unchecked Sendable {
    ...
    // KV 캐시 재사용을 위한 현재 위치 조회
    public var currentKVPosition: Int {
        return lastArgmaxPosition + 1
    }
    ...
    public func runPrefill(
        ...
        if isMonolithic {
            return try await runMonolithicPrefill(on: &contextTokens, contextPos: contextPos, tokenizer: tokenizer, positionOffset: positionOffset)
        }
        var batchPos = 0
        ...
        while batchPos + batchSize <= contextPos {
            ...
            // Generate position IDs (positionOffset으로 KV 캐시 재사용 시 올바른 위치 지정)
            let positionIds = try MLMultiArray(shape: [NSNumber(value: batchSize)], dataType: .int32)
            for i in 0..<batchSize {
                positionIds[i] = NSNumber(value: positionOffset + batchPos + i)
            }
            ...
            // Set causal attention pattern (positionOffset 적용)
            for i in 0..<batchSize {
                for j in 0..<contextLength {  // Use full contextLength
                    if j <= (positionOffset + batchPos + i) {
        ...
        if batchPos < contextPos {
            ...
            while batchPos < contextPos {
                ...
                let _ = try await generateNextToken(
                    ...
                    currentPos: positionOffset + batchPos + 1,  // generateNextToken uses 1-indexed positions
                    temperature: 0,
                    tokenizer: tokenizer
                )
                if debugLevel >= 1 {
                    print("  Prefill single token at pos \(positionOffset + batchPos): \(contextTokens[batchPos])")
                }
                batchPos += 1
            }
        }

        // positionOffset을 포함한 최종 위치 반환
        return positionOffset + contextPos
    }
    ...
    private func runMonolithicPrefill(
        ...
        while batchPos < contextPos {
            ...
            // Generate position IDs for full batch (positionOffset으로 KV 캐시 재사용 지원)
            let positionIds = try MLMultiArray(shape: [NSNumber(value: batchSize)], dataType: .int32)
            for i in 0..<batchSize {
                positionIds[i] = NSNumber(value: positionOffset + batchPos + i)
            }
            ...
            // Create current_pos as tensor (positionOffset 적용)
            let currentPosArray = try MLMultiArray(shape: [1], dataType: .int32)
            currentPosArray[0] = NSNumber(value: positionOffset + batchPos)
        ...
        while batchPos < contextPos {
            ...
            // Single position ID (positionOffset 적용)
            let positionIds = try MLMultiArray(shape: [1], dataType: .int32)
            positionIds[0] = NSNumber(value: positionOffset + batchPos)

            // Single-token causal mask (positionOffset 적용)
            let singleMask = try MLMultiArray(
                shape: [1, 1, 1, NSNumber(value: contextLength)],
                dataType: .float16
            )
            for j in 0..<contextLength {
                singleMask[[0, 0, 0, j] as [NSNumber]] = j <= (positionOffset + batchPos)
                    ? NSNumber(value: Float(0.0))
                    : NSNumber(value: Float(-Float.infinity))
            }

            // Current position (positionOffset 적용)
            let currentPosArray = try MLMultiArray(shape: [1], dataType: .int32)
            currentPosArray[0] = NSNumber(value: positionOffset + batchPos)
        ...
        // After prefill, positions 0..(positionOffset+contextPos-1) should be visible
        let totalPos = positionOffset + contextPos
        if argmaxInModel {
            if let maskBuffer = argmaxCausalMaskBuffer {
                ...
                if let baseAddress = CVPixelBufferGetBaseAddress(maskBuffer) {
                    ...
                    for j in 0..<min(totalPos, contextLength) {
                        ptr[j] = Float16(0.0)
                    }
                }
                CVPixelBufferUnlockBaseAddress(maskBuffer, [])
                lastArgmaxPosition = totalPos - 1
    ...
    public func generateResponse(
        ...
        // KV 캐시 관리: preserveKVCache=true이면 기존 상태 유지 (멀티턴 KV 캐시 재사용)
        let kvCacheOffset: Int
        if preserveKVCache && lastArgmaxPosition >= 0 {
            // KV 캐시 보존: 기존 위치 이후부터 새 토큰 prefill
            kvCacheOffset = lastArgmaxPosition + 1
            if debugLevel >= 1 {
                print("[KV Cache] 재사용 모드: offset=\(kvCacheOffset)")
            }
        } else {
            // 기본 동작: KV 캐시 초기화
            kvCacheOffset = 0
            ...
            if disablePrefill {
                ...
                while pos < contextTokens.count {
                    let _ = try await generateNextToken(
                        ...
                        currentPos: kvCacheOffset + pos + 1,
                        temperature: 0,
                        tokenizer: tokenizer
                    )
                    if debugLevel >= 1 {
                        print("  Debug infer prefill token at pos \(kvCacheOffset + pos): \(contextTokens[pos])")
                    }
                    pos += 1
                }
                currentPos = kvCacheOffset + contextTokens.count
                prefillTime = 0
            } else {
                // Run prefill with mutable copy (kvCacheOffset으로 KV 캐시 재사용 위치 지정)
                currentPos = try await runPrefill(on: &contextTokens, contextPos: contextTokens.count, tokenizer: tokenizer, positionOffset: kvCacheOffset)
                prefillTime = CFAbsoluteTimeGetCurrent() - startTime
            }

            // KV 캐시 위치 추적: argmaxInModel=false 경로 또는 maxTokens=0 호출에서도
            // prefill 종료 위치를 lastArgmaxPosition에 반영해야 후속 generateResponse가
            // preserveKVCache=true로 호출됐을 때 KV가 정상 보존된다.
            // monolithic+argmaxInModel=true 경로에서는 runMonolithicPrefill 내부가 이미
            // 동일 값으로 갱신하므로 idempotent.
            if currentPos > 0 {
                lastArgmaxPosition = max(lastArgmaxPosition, currentPos - 1)
            }
